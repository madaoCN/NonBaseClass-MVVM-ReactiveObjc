...
{
    NSTimeInterval showTime = [SVProgressHUD fk_showTimeWithStatus:status];
...
#pragma mark - private
+ (NSTimeInterval)fk_showTimeWithStatus:(NSString *)status
{
    if (!status) {
        return kMaxShowTime;
    }
    // 每个字 0.3s 最低三秒
    return MIN(MAX(status.length * 0.3, 3.0f), 3.0f);
}
